	...
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form class="form-horizontal" id="saveList" action="<%=path%>/back/addtype" method="post" >
					<div class="form-group">
						<label class="col-sm-5 control-label">类别名称</label>
						<div class="col-sm-10">
							<input type="text" class="form-control" id="typename" name="typename" placeholder="请输入类别名称" required="required">
						</div>
					</div>
				</form>
			</div>
			...
			<div class="modal-footer">
				<button type="submit" class="btn btn-outline-primary" id="btn-add" form="saveList">创建类别</button>
					...
					<tbody id="blog_addhere">
						<c:forEach items="${reslist}" var="type">
							<tr>
								...
								<th> 
									<div class="inline"> 
										<button type="button" class="btn btn-sm btn-outline-success btn-res" name="${type.id }">恢复</button> 
									</div> 
								</th>
							</tr>
						</c:forEach>
                ...
                <div class="row">
                    <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                        <div class="page-header">
                            <h2 class="pageheader-title">类别管理</h2>
                        </div>
                    </div>
                </div>
                <!-- end pageheader -->
                    ...
                    <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
						...
						<div class="card">						
							<div class="card-header p-4">
								<button type="button" class="btn btn-outline-primary" data-toggle="modal" data-target="#saveModal">添加类型</button>
								<button type="button" class="btn btn-outline-dark ml-3" data-toggle="modal" data-target="#restoreModal">恢复类型</button>
							</div>
	...
	<script type="text/javascript">
		$(window).on('load', function(){
			...
			$(".btn-del").click(function() {
				...
				if (confirm("确认要删除吗？")) {
				    window.location.href="<%=path%>/back/deletetype?typeid="+strid
				  } else {
				    alert("已取消");
				  }
			});
			...
			$(".btn-res").click(function() {
				var strid=$(this).attr("name");
				if (confirm("确认要恢复吗？")) {
				    window.location.href="<%=path%>/back/undodeltype?typeid="+strid
				  } else {
				    alert("已取消");
				  }
			});
